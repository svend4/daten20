...
def email_manager(temp_db):
    ...
    return EmailVerificationManager(
        db_path=temp_db, email_notifier=None, base_url="http://test.local"  # Disable email sending for tests
    )
...
class TestEmailVerificationManager:
    ...
    def test_generate_verification_token(self, email_manager, temp_db):
        ...
        token = email_manager.generate_verification_token(user_id=1, email="test@example.com", ip_address="127.0.0.1")
    ...
    def test_send_verification_email_without_notifier(self, email_manager):
        ...
        success, result = email_manager.send_verification_email(
            user_id=1, email="test@example.com", username="testuser", ip_address="127.0.0.1"
        )
    ...
    def test_generate_password_reset_token(self, email_manager, temp_db):
        ...
        token = email_manager.generate_password_reset_token(user_id=1, email="test@example.com", ip_address="127.0.0.1")
    ...
    def test_send_password_reset_email_without_notifier(self, email_manager):
        ...
        success, result = email_manager.send_password_reset_email(email="test@example.com", ip_address="127.0.0.1")
    ...
    def test_send_password_reset_email_nonexistent_user(self, email_manager):
        ...
        success, message = email_manager.send_password_reset_email(
            email="nonexistent@example.com", ip_address="127.0.0.1"
        )
    ...
    def test_verify_reset_token_expired(self, email_manager):
        ...
        token = email_manager.generate_password_reset_token(1, "test@example.com", expires_in=-3600)
    ...
    def test_reset_password_success(self, email_manager, temp_db, bcrypt_instance):
        ...
        success, message = email_manager.reset_password(
            token=token, new_password="new_password_123", bcrypt_instance=bcrypt_instance, ip_address="127.0.0.1"
        )
    ...
    def test_reset_password_invalid_token(self, email_manager, bcrypt_instance):
        ...
        success, message = email_manager.reset_password(
            token="invalid_token_123", new_password="new_password_123", bcrypt_instance=bcrypt_instance
        )
    ...
    def test_reset_password_used_token(self, email_manager, bcrypt_instance):
        ...
        token = email_manager.generate_password_reset_token(1, "test@example.com")
        email_manager.reset_password(token=token, new_password="new_password_123", bcrypt_instance=bcrypt_instance)

        # Try to use again
        success, message = email_manager.reset_password(
            token=token, new_password="another_password", bcrypt_instance=bcrypt_instance
        )
...
class TestIntegrationScenarios:
    ...
    def test_complete_email_verification_workflow(self, email_manager):
        ...
        success, token = email_manager.send_verification_email(user_id=1, email="test@example.com", username="testuser")
        assert success is True
    ...
    def test_complete_password_reset_workflow(self, email_manager, bcrypt_instance):
        ...
        success, token = email_manager.send_password_reset_email(email="test@example.com")
        assert success is True
        ...
        # 3. Reset password
        success, message = email_manager.reset_password(
            token=token, new_password="secure_new_password_123", bcrypt_instance=bcrypt_instance
        )
        assert success is True

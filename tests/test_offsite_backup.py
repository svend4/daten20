...
class TestS3BackupProvider:
    ...
    def test_upload_backup(self, mock_boto_client, temp_backup_file):
        ...
        success = provider.upload_backup(temp_backup_file, "backups/test_backup.tar.gz", metadata={"test": "value"})
...
class TestGCSBackupProvider:
    ...
    def test_init(self, mock_gcs_client):
        ...
        provider = GCSBackupProvider(bucket_name="test-bucket", project_id="test-project")
...
class TestOffsiteBackupManager:
    ...
    def test_init_gcs_provider(self, mock_gcs_client):
        ...
        manager = OffsiteBackupManager(
            provider_type="gcs", provider_config={"bucket_name": "test-bucket"}, encryption_password="test_password"
        )
    ...
    def test_init_invalid_provider(self):
        ...
        with pytest.raises(ValueError, match="Unsupported provider"):
            OffsiteBackupManager(provider_type="invalid", provider_config={})
    ...
    def test_upload_backup_without_encryption(self, mock_boto_client, temp_backup_file):
        ...
        manager = OffsiteBackupManager(provider_type="s3", provider_config={"bucket_name": "test-bucket"})

        success, remote_key = manager.upload_backup(temp_backup_file, encrypt=False, compress=False)
    ...
    def test_upload_backup_with_encryption(self, mock_boto_client, temp_backup_file, temp_dir):
        ...
        manager = OffsiteBackupManager(
            provider_type="s3", provider_config={"bucket_name": "test-bucket"}, encryption_password="test_password"
        )

        success, remote_key = manager.upload_backup(temp_backup_file, encrypt=True, compress=False)
    ...
    def test_cleanup_old_backups(self, mock_boto_client):
        ...
        manager = OffsiteBackupManager(
            provider_type="s3", provider_config={"bucket_name": "test-bucket"}, retention_days=30
        )
...
class TestIntegrationScenarios:
    ...
    def test_full_backup_restore_workflow(self, mock_boto_client, temp_backup_file, temp_dir):
        ...
        # Create manager
        manager = OffsiteBackupManager(
            provider_type="s3", provider_config={"bucket_name": "test-bucket"}, encryption_password="test_password_123"
        )

        # Upload backup
        success, remote_key = manager.upload_backup(
            temp_backup_file, backup_name="test_backup.tar.gz", encrypt=True, compress=False
        )

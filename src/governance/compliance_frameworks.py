    ...
    async def get_compliance_score(
        ...
        for assessment in self.assessments.values():
            if assessment.control_id in [c.control_id for c in controls]:
                if (assessment.control_id not in control_status or
                    assessment.assessment_date > control_status[assessment.control_id].assessment_date):
        ...
        for control in controls:
            ...
            if assessment and assessment.status in [ControlStatus.NOT_IMPLEMENTED, ControlStatus.IN_PROGRESS]:
                gaps.append({
                    ...
                    "remediation_due": assessment.remediation_due_date.isoformat() if assessment.remediation_due_date else None
                })
        ...
        for other in self.controls.values():
            if control_id in other.mapped_controls:
                reverse_mapped.append({
                    ...
                    "framework": other.framework.value,
                    "title": other.title
                })

...
class PerformanceMonitor:
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        http_stats = self.collector.calculate_statistics(
            'http_request_duration_ms',
            start_time=one_hour_ago
        )

        db_stats = self.collector.calculate_statistics(
...
class ResourceMonitor:
    ...
    def collect_metrics(self):
        ...
        self.collector.record_counter('system_network_bytes_received', random.uniform(1e6, 1e9), {'direction': 'received'})
...
class AlertManager:
    ...
    def _trigger_alert(self, rule: Dict[str, Any], current_value: float):
        ...
        existing_alert = next(
            (a for a in self.alerts.values() if a.name == rule['name'] and a.status == AlertStatus.ACTIVE),
            None
        )
...
class MonitoringEngine:
    ...
    def _setup_default_health_checks(self):
        ...
        self.health_check_manager.register_health_check(
            'database',
            lambda: True  # Would check actual database connection
        )

        self.health_check_manager.register_health_check(
            'cache',
            lambda: True  # Would check cache connection
        )

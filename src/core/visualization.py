...
class ChartGenerator:
    ...
    def generate_service_distribution_by_region(
        self,
        services: List[Any],
    ...
    def generate_rate_distribution(
        self,
        services: List[Any],
        output_file: Optional[str] = None
    ) -> str:
    ...
    def generate_interactive_dashboard(
        ...
        fig.add_trace(
            go.Bar(x=list(regions), y=list(counts), name='Services', marker_color='#0d6efd'),
            row=1, col=1
        )
        ...
        fig.add_trace(
            go.Histogram(x=rates, name='Rates', marker_color='#198754', nbinsx=20),
            row=1, col=2
        )
        ...
        # Update layout
        fig.update_layout(
            title_text="Document Management System - Analytics Dashboard",
            showlegend=False,
            height=800
        )
    ...
    def generate_3d_scatter(
        ...
        fig = go.Figure(data=[go.Scatter3d(
            ...
            text=[f"{name}<br>Region: {region}<br>Rate: €{rate:.2f}"

...
class AdvancedAnalytics:
    ...
    def _calculate_summary(self, services: List[Dict]) -> Dict:
        ...
        total_monthly_cost = sum(
            s.get('brutto_rate', 0) * s.get('hours_per_month', 0)
            for s in services
        )
    ...
    def _generate_insights(
        ...
        if services_by_region:
            ...
            insights.append(
                f"🏆 Наиболее популярный регион: {top_region[0]} ({top_region[1]} услуг)"
            )
        ...
        if anomalies:
            ...
            if high_severity:
                insights.append(
                    f"⚠️ Обнаружено {len(high_severity)} аномальных значений высокой важности"
                )
    ...
    def _generate_recommendations(
        ...
        if services_by_region and len(services_by_region) < 5:
            recommendations.append(
                "🌍 Рассмотрите расширение географического покрытия - "
                "вы работаете только в нескольких регионах"
            )

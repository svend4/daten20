...
class AnalyticsEngine:
    ...
    def _generate_recommendations(self, services: List[Service]) -> List[str]:
        ...
        if rates:
            ...
            if rate_range > 20:
                recommendations.append(
                    f"Большой разброс ставок ({rate_range:.2f} €). Рассмотрите стандартизацию."
                )
        ...
        if len(by_region) < 3:
            recommendations.append(
                "Услуги представлены в малом количестве регионов. Рассмотрите расширение."
            )
        ...
        if umlages_count > 0 and reserve_count > 0:
            recommendations.append(
                "Используются разные режимы расчета. Рассмотрите унификацию для упрощения."
            )
            ...
            try:
                ...
                forecasts.append({
                    ...
                    'change_percent': self._calculate_change_percent(
                        ...
                        float(breakdown.final_hourly_rate)
                    )
                })
            except Exception as e:
